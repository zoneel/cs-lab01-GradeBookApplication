...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (type == "standard")
            {
                BaseGradeBook gradeBook = new StandardGradeBook(name,isitweighted);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else if(type == "ranked")
            {
                BaseGradeBook gradeBook = new RankedGradeBook(name, isitweighted);
                Console.WriteLine("Created gradebook {0}.", name);
                GradeBookUserInterface.CommandLoop(gradeBook);
            }
            else
            {
                Console.WriteLine($"{type} is not a supported type of gradebook, please try again\" to the console, then escape the method.");
                return;
            }
